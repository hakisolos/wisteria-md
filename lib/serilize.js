...
function serializeMessage(msg, sock) {
	...
	if (quoted) {
		...
		async function resolveLidToJid(participant, groupJid, sock) {
			if (!participant.endsWith('@lid')) return participant;
		
			try {
				const metadata = await sock.groupMetadata(groupJid);
				const realJid = metadata.participants.find(p => p.id.endsWith(participant.replace('@lid', '@s.whatsapp.net')));
				return realJid?.id || participant; 
			} catch (err) {
				console.error('Failed to resolve lid to JID:', err);
				return participant; 
			}
		}
