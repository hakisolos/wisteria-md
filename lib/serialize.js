...
async function serializeMessage(msg, sock) {
	...
	m.lidtojid = async (lid) => {
		const metadata = await checkGmd(m.jid); 
	...
	m.groupMetadata = async (jid, msg) => {
		const data = await checkGmd(jid);

		const content = data
			? JSON.stringify(data, null, 2)
			: "Not found ❌";

		return await sock.sendMessage(m.jid, { text: content }, { quoted: msg });
	};
