...
class WordRaceManager {
  ...
  createGame(groupJid, player1, player2) {
    ...
    return {
      success: true,
      message: `Word Race Game created between @${player1.split('@')[0]} and @${player2.split('@')[0]}`,
      gameId,
      gameState
    };
  }
  ...
  generateChallenge(level) {
    ...
    const startingLetter = letters.charAt(Math.floor(Math.random() * letters.length));
  ...
  async makeMove(groupJid, playerId, word) {
    ...
    if (!gameState) {
      return {
        success: false,
        message: "You're not in an active game. Start one by replying to someone with !wrg"
      };
    }
    ...
    if (gameState.level > 3) {
      ...
      gameState.roundTimeLimit = Math.max(30 * 1000, this.defaultTimeout - (reductionFactor * 30 * 1000));
    }
  ...
  async validateWord(word) {
    try {
      ...
      const response = await axios.get(`${this.dictionaryApiUrl}${word.toLowerCase()}`);
  ...
  checkTimeouts(m) {
    ...
    for (let i = this.timeoutResults.length - 1; i >= 0; i--) {
      ...
      if (result.groupJid === m.jid) {
        ...
        m.client.sendMessage(result.groupJid, {
          text: `⏰ Time's up! @${result.loser.split('@')[0]} took too long to respond.\n\n🏆 @${result.winner.split('@')[0]} wins with ${result.scores[result.winner]} points!\n@${result.loser.split('@')[0]} had ${result.scores[result.loser]} points.`,
          mentions: [result.winner, result.loser]
        }).catch(err => console.error('Error sending timeout notification:', err));
  ...
  endGame(groupJid, playerId) {
    ...
    return {
      success: true,
      message: `Game ended by @${playerId.split('@')[0]}.\n\nFinal scores:\n@${playerId.split('@')[0]}: ${scores[playerId]} points\n@${opponent.split('@')[0]}: ${scores[opponent]} points`,
      opponent,
      scores
    };
  }
...
nikka(
  ...
  async (m) => {
    try {
      ...
      // Create new game
      const result = wordRaceManager.createGame(m.jid, m.sender, m.quoted.sender);
...
nikka(
  ...
  async (m) => {
    try {
      ...
      await m.client.sendMessage(m.jid, {
        text: `🎮 *WORD RACE GAME* 🎮\n\n✅ @${m.sender.split('@')[0]} submitted: "${moveResult.validWord}"\n\n📊 Scores:\n@${gameInfo.gameState.players[0].split('@')[0]}: ${moveResult.scores[gameInfo.gameState.players[0]]} points\n@${gameInfo.gameState.players[1].split('@')[0]}: ${moveResult.scores[gameInfo.gameState.players[1]]} points\n\n🆙 Level: ${moveResult.level}\n\n🎯 New challenge for @${moveResult.nextPlayer.split('@')[0]}:\n${moveResult.nextChallenge.description}\n\n⏱️ Time limit: ${moveResult.timeLimit} seconds`,
