    ...
    try {
      ...
      try {
        ...
        if (!aiError.message.includes('network') && 
          ...
          await m.reply('Gomen nasai! I encountered an error. Please try again later~ 😔');
        }
      }
    } catch (error) {
  ...
  async (m, { eventType }) => {
    try {
      ...
      try {
        ...
        console.error('Error getting AI response:', aiError);
        if (!aiError.message.includes('ECONNREFUSED') && 
            !aiError.message.includes('timeout')) {
          await m.reply('Gomen nasai! I encountered an error while processing your message. 😔');
        }
      }
    } catch (error) {
